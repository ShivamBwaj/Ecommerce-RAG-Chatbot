...
def parse_function_definition(function_def: str) -> Dict[str, Any]:
    ...
    if docstring:
        ...
        result["description"] = docstring[:desc_end].strip() if desc_end > 0 else docstring.strip()  # Fixed: [desc_end] -> [:desc_end]

        # Parse parameter descriptions
        param_desc = parse_docstring_params(docstring)  # Fixed: variable name consistency

        # Extract return description
        if "Returns:" in docstring:
            result["returns"]["description"] = docstring.split("Returns:")[1].strip().split('\n')[0]  # Fixed: result["returns"] -> result["returns"]["description"]
    ...
    for i, arg in enumerate(args.args):  # Fixed: args -> args.args
        if arg.arg == 'self':
            continue

        param_info = {
            "type": get_type_from_annotation(arg.annotation) if arg.annotation else "string",
            "description": param_desc.get(arg.arg, "")  # Fixed: param_descss -> param_desc
        }
...
def get_tool_descriptions(function_list):
    ...
    for function in function_list:
        ...
        result = parse_function_definition(function_string)  # Note: parse_function_definition is not defined
